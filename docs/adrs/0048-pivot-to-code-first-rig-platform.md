...
- [ ] All ADRs listed under `Supersedes` (the proposed batch) have
      their **status field** updated in-file from `Proposed` to
      `Superseded by 0048`. Their bodies are **not** edited; they
      stand as historical context.
- [ ] [`README.md`](README.md) ADR index reflects the supersedes
      mapping in the `Status` column for each affected row.
- [ ] [`README.md`](README.md) ADR index has new rows for ADRs
      0048–0057 (status `Proposed`, phase per the new phasing below).
- [ ] [`README.md`](README.md) decision graph drops the 0033/0038/
      0040/0041/0042/0043/0044/0045 cluster and adds the 0048 →
      0049–0057 fan-out plus the rig (0047) → 0051/0052/0053/0054
      edges.
- [ ] [`README.md`](README.md) `Mapping-to-SAM summary` section is
...
- [ ] A short note appended to [`AGENTS.md`](../../AGENTS.md) §1
...
- [ ] [`docs/adrs/metrics.csv`](metrics.csv) row appended for 0048.
- [ ] No file under `crates/` is edited as part of accepting this
...
- **Pivot to rig + roll our own platform shape (no Mastra parity).**
  Rejected. We tried that; it produced 18 ADRs and the director
  said too early. Mastra's shape is validated by an actual user
  base; copying it gives us a credibility shortcut and a free
  contributor surface (people who already think in Mastra concepts
  can read ork ADRs without translation).
...
The README phase table currently runs 1–4 with the old SAM-parity
buckets. After this ADR the phases become:
...
The README index update covers this rewrite.
...
- **Adversarial review** (AGENTS.md §7). This is a strategy ADR
  with downstream commitments larger than any single ADR in the
  set; an adversarial pass should run before flipping 0048 from
  `Proposed` to `Accepted`. The skeptic should attack: (1)
  whether Mastra parity is the right anchor or just the loudest
  recent project, (2) whether rig will still be the right engine
  in 18 months given its 0.x cadence, (3) whether on-prem Rust
  customers are the same buyers as code-first agent-platform
  buyers, and (4) whether the 0.5–1y of work implied by
  0049–0057 displaces something a current pilot needs.
