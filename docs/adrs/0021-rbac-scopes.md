...
Implementation phases A–E shipped together (`feat(adr0021): RBAC scopes`).
Phases A (ScopeChecker + vocabulary), B (HTTP route gates), C (Agent +
Tool boundaries + cross-tenant `agent_call`), D (artifact decorator +
`webui:access` + `TenantSettings.scope_allowlist` schema), and E
(config defaults seed list).

| Severity | Finding | Resolution |
| -------- | ------- | ---------- |
| Critical | `ScopeCheckedArtifactStore` was shipped under `crates/ork-storage/` but never wired — production paths still handed the raw `Arc<dyn ArtifactStore>` to handlers. The `artifact:context-<id>:*` per-context narrowing the ADR depends on was therefore unenforced; only the route-level `artifact:tenant:read` gated reads. | Fixed in-session: `routes/artifacts.rs` and `routes/a2a.rs` now wrap `state.artifact_store` per request via a new `scoped_artifact_store(state, auth)` helper that builds a `ScopeCheckedArtifactStore` keyed on `auth.scopes`. The wrapped store flows into `AgentContext.artifact_store` for both `message/send` and `message/stream` handlers. `Forbidden` errors from the wrapper surface as 403; non-auth errors keep the existing 404 mapping. |
| Major | `ScopeChecker::require` originally returned `OrkError::Forbidden` without emitting the `audit.scope_denied` event ADR §`Audit` mandates. Two of the three boundaries (tool plane, artifact decorator) and the in-tenant `agent_call` deny path silently denied. | Fixed in-session: `ScopeChecker::require` now emits `tracing::info!(event = "audit.scope_denied", scope = …, …)` on every deny path. New `ork_security::audit` module publishes `SCOPE_DENIED_EVENT` and `SENSITIVE_GRANT_EVENT` constants; `require_scope!`, `agent_call.rs`, and `webui::routes` all migrated onto the constants. |
| Major | The "End-user" default scope set in `config/default.toml` and the `auth_for` test helper carried `agent:*:cancel`, but ADR §`Defaults` table reserves `agent:<id>:cancel` for Operator/admin tokens. | Fixed in-session: `agent:*:cancel` removed from `auth_for` and from `[security.scopes].end_user`. Cancel-test fixtures (`a2a_dispatcher.rs`, `push_outbox_delivery.rs`) mint the scope explicitly via `auth_for_with_scopes`. New deny test `tasks_cancel_without_cancel_scope_is_forbidden` pins the gate. |
| Major | The ADR text reads "the registry wraps every `Agent::send` / `send_stream`", but the implementation puts the `agent:<id>:invoke` gate at the HTTP entry points only. Direct `Agent::send` callsites — workflow engine step execution, future internal subsystem calls — bypass the gate. | Acknowledged, deferred. The HTTP edge covers all production traffic today; the workflow engine's `system_runtime_caller` carries the wildcard scopes the engine needs, and there are no other production callsites. A `ScopeCheckedAgent` decorator (analogous to the artifact wrapper) is the natural next step and is logged as a follow-up. |
| Minor | `is_local()` is over-broad as a "cross-tenant" trigger — a remote agent might still belong to the same tenant (sibling ork process), so `agent_call` over-rejects in-tenant remote delegation. | Fixed in-session: `agent_registry::is_local` doc-comment now spells out the conservatism and points at the follow-up that adds `target_card.tenant_id` to `RemoteAgentEntry`. The behaviour is unchanged. |
| Minor | Workflows / schedules / tenants routes did not migrate to `require_scope!`. Tenants still calls `ctx.has_scope(...)` directly; workflows has no scope gate beyond tenant isolation. | Acknowledged, deferred. The existing tenant CRUD checks meet ADR §`Tenant CRUD restricted` semantics already. ADR-0021 does not coin a `workflow:<…>:<…>` scope, and ADR-0019 (schedules) is still Proposed — the `schedule:read|write` constants are reserved for that ADR. Both follow-ups will move onto the macro. |
| Minor | `ScopeChecker::validate_format` is exported but never invoked. The `TenantSettings.scope_allowlist` write path accepts arbitrary strings, including the `*:*:*` shape `validate_format` was written to reject. | Acknowledged, deferred. `update_settings` plumbing (calling `validate_format` on each entry and returning `OrkError::Validation` on the first failure) is a straightforward follow-up commit; the model-side schema and the `[security.scopes]` config seed list are in place. |
| Minor | `tool:peer_*:invoke` is gated at the peer arm of `ToolPlaneExecutor` even though ADR §`Vocabulary` does not register the `tool:peer_<agent>_<skill>:invoke` shape. The inner `agent:<target>:delegate` check in `agent_call.rs` is the canonical gate. | Acknowledged. The peer-arm gate is the coarse "is this caller allowed to use peer tooling?" check (granted by `tool:*:invoke`); the fine-grained agent-target check happens after id resolution. The ADR vocabulary table will be widened in the same follow-up that migrates workflows + schedules. |
| Minor | LLM model gate (`model:<provider>:<model>:invoke`) is not wired in `LlmRouter::resolve`. ADR §`Decision points` step 5 marks it "optional but recommended"; the diff documents the `[security.enforce_model_scopes]` config anchor but the runtime check is absent. | Acknowledged, deferred. The check needs `caller.scopes` plumbed through `ResolveContext` (or a new explicit param on `resolve()`); both touch the existing tenant-resolver task-locals. |
| Minor | Per-gateway `gateway:<id>:invoke` checks (`crates/ork-gateways/`) were not added. | Acknowledged, deferred. Touches the four built-in adapters (`rest`, `webhook`, `event_mesh`, `mcp`) plus the noop adapter. The webui gateway is gated via `webui:access` already. |
| Minor | `TenantSettings.scope_allowlist` defence-in-depth intersection in `auth_middleware` is not implemented. Today the field is schema-only. | Acknowledged, deferred. The intersection requires a tenant-scoped lookup on every request; will land alongside the `validate_format` write-path call so misconfigured allowlists fail loud at write time, not silently at intersect time. |
| Nit | `[security.scopes]` placeholder strings (`<self>`, `<intersected_caller_scopes>`, `<that-agent>`) sat inside what looked like real TOML arrays — a reader copy-pasting verbatim would have minted the placeholder as a literal scope. | Fixed in-session: `service_agent` and `external_a2a_partner` blocks moved into TOML comments so the placeholders cannot be loaded as canonical scopes. |
| Nit | The `event = "audit.scope_denied"` / `"audit.sensitive_grant"` strings were duplicated across `ork-api` middleware, `ork-integrations`, `ork-storage`, `ork-webui`. | Fixed in-session: pulled into `ork_security::audit::{SCOPE_DENIED_EVENT, SENSITIVE_GRANT_EVENT}` constants. |
| Nit | `crates/ork-storage/src/scoped.rs` doc-comment warned about the per-request wiring footgun while no production callsite actually wired it. | Fixed in-session: the wiring landed (Critical row above), so the doc comment now describes the real production setup rather than warning about hypothetical misuse. |
