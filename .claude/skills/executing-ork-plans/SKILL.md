...
Follow test-driven development for any new behaviour: failing test →
minimal impl → green → refactor → commit. For pure refactors or rename
ADRs, TDD is a weaker fit; use judgement.
...
All three must be green. If `cargo clippy` is not configured for this
crate, skip it and note that in the session.
...
green, dispatch the `code-reviewer` subagent (defined at
...
session has an isolated working directory. The verification gate runs
**per worktree**.
