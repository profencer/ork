...
/// True when `tool_name` is one of the ADR-0016 artifact tools.
pub(crate) fn is_artifact_tool(name: &str) -> bool {
    ARTIFACT_TOOL_NAMES.contains(&name)
}

/// JSON-RPC / LLM tool catalog entries for the seven artifact tools.
#[must_use]
pub fn artifact_tool_descriptors() -> Vec<ToolDescriptor> {
    ARTIFACT_TOOL_NAMES
        .iter()
        .filter_map(|n| integration_descriptor(n))
        .collect()
}

/// For [`super::tools::IntegrationToolExecutor::descriptor`].
pub(crate) fn integration_descriptor(name: &str) -> Option<ToolDescriptor> {
    match name {
        "create_artifact" => Some(ToolDescriptor {
            name: name.into(),
            description: "Create a versioned named blob (text or base64) in tenant/task scope."
                .into(),
        ...
        "append_artifact" => Some(ToolDescriptor {
            name: name.into(),
            description: "Append bytes to the latest version and write a new version."
                .into(),
            parameters: serde_json::json!({
                "type": "object",
                "properties": {
        ...
        "list_artifacts" => Some(ToolDescriptor {
            name: name.into(),
            description: "List artifacts in the current scope; optional name prefix and label filter."
                .into(),
            parameters: serde_json::json!({
                "type": "object",
                "properties": {
        ...
        "load_artifact" => Some(ToolDescriptor {
            name: name.into(),
            description: "Load an artifact: inline text/data if small, else file URI (presign or API proxy URL)."
                .into(),
            parameters: serde_json::json!({
                "type": "object",
                "properties": {
        ...
        "artifact_meta" => Some(ToolDescriptor {
            name: name.into(),
            description: "Return JSON metadata (mime, size, labels) for a named version."
                .into(),
            parameters: serde_json::json!({
                "type": "object",
                "properties": {
        ...
        "delete_artifact" => Some(ToolDescriptor {
            name: name.into(),
            description: "Delete one version, all versions, or use version: \"*\" for all."
                .into(),
            parameters: serde_json::json!({
                "type": "object",
                "properties": {
        ...
        "pin_artifact" => Some(ToolDescriptor {
            name: name.into(),
            description: "Add `pinned=true` to an artifact (retention sweeps ignore pinned)."
                .into(),
            parameters: serde_json::json!({
                "type": "object",
                "properties": {
