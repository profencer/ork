...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Webui { cmd } => match cmd {
            WebuiCommand::Dev { vite_port } => {
...
/// `ork webui dev` — Vite + `ork-server` with `WEBUI_DEV_PROXY`, `ORK_CONFIG_EXTRA` (webui gateway), and
/// `ORK_A2A_PUBLIC_BASE` (ADR-0017).
async fn run_webui_dev(vite_port: u16) -> Result<()> {
